...
The previous workflow set the mutable `:latest` reference directly. Setting the
same tag did not necessarily change the pod template, so pods could continue
running an older build after the tag moved.
...
The replacement Role is declared in `k8s/stable-ci-rbac.yaml` and the repository
uses only `KUBECONFIG_STABLE_CI` for stable deployment access.
